...
export const notificationReducer = createReducer(
  {
    ...
    [actions.getAllNotificationSuccessful]: (state, payload) => {
      notifications =
        payload.page > 1
    ...
    [actions.getUnreadCountPending]: (state) =>
      updateObject(state, { isloading: true }),
    [actions.getUnreadCountSuccessful]: (state, payload) =>
      updateObject(state, {
        isloading: false,
    ...
    [actions.clearNotifications]: (state, payload) =>
      updateObject(state, {
        isloading: false,
        notificationCount: 0,
        notifications: [],
    ...
    [actions.acceptRequestPending]: (state, payload) =>
      updateObject(state, { isloading: true }),
...
const updateNotifications = (state, payload) => {
  ...
  notifications.splice(
    notifications.findIndex(
      (e) => e.follow && e.follow.id === payload.notification_id
    ),
    1
  );
};
