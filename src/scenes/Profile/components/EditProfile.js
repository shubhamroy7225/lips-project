...
const EditProfile = ({setIsEdit, user}) => {
  ...
  const updateUserProfile = (e) => {
    let bio = userBio.bio;
    const {show_following, show_followers, header_image, photo_url} = userForm;
    setUserForm({...userForm, ['bio']: userForm.bio});
                  ...
                  <label htmlFor="file_input">
                    <figure  className="lps_fig lps_fig90">
                      ...
                      {files.header_image.src ?  <img src={files.header_image.src} alt="Add Image" /> :
                          (userForm.header_images && userForm.header_images.medium ? <img src={userForm.header_images.original} alt="Add Image" /> : <img src={require("assets/images/icons/image_icon_dashed.svg")} alt="Add Image" className="lps_img_fixed" />) }
                    </figure>
                  </label>
                </div>
