...
export default () => {
  ...
  const addFavoriteTags = () => {
    if (user) actions.setFavoriteAvoidTags({hashtags: {hide: selectTags}}).then(res => {
      if(res) history.push("/");
    });
    else actions.setFavoriteAvoidTagsJustBrowse({hashtags: {hide: selectTags}}).then(res => {
      if(res) history.push("/main-feeds");
    });
  };
  return (
